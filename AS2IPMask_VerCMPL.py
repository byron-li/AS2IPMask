...
def whois(query):
	...
	try:
		...
		match = pattern.search(response.decode())
		if (match):
...
def fill_blank(asn, asn_info_dict):
	response = whois(asn)

	if(response):
		asn_info_dict["as_name"] = response
...
def lookup_asn_info(asn):
	...
	if asn in asn_info.keys():
		...
		if not asn_item_info["as_name"]:
			tmp = fill_blank(asn, asn_item_info)

	else:
		asn_info[asn] = fill_blank(asn, asn_item_info)
